...
const headerRoomCodeEl = document.getElementById('header-room-code')!;
...
const btnSettingsNav = document.getElementById('btn-settings') as HTMLButtonElement;
const btnGesturesNav = document.getElementById('btn-gestures') as HTMLButtonElement;
const pageSettings = document.getElementById('page-settings')!;
const pageGestures = document.getElementById('page-gestures')!;

type PageView = 'main' | 'settings' | 'gestures';

function showPage(page: PageView) {
  const onMain = page === 'main';
  headerConnected.style.display = onMain ? 'flex' : 'none';
  document.getElementById('connected-ui')!.style.display = onMain ? 'block' : 'none';
  navBtns.style.display = onMain ? 'flex' : 'none';
  logEl.style.display = onMain ? 'block' : 'none';
  pageSettings.style.display = page === 'settings' ? 'block' : 'none';
  pageGestures.style.display = page === 'gestures' ? 'block' : 'none';
}

btnSettingsNav.addEventListener('click', () => showPage('settings'));
btnGesturesNav.addEventListener('click', () => showPage('gestures'));
(document.getElementById('btn-back-settings') as HTMLButtonElement).addEventListener('click', () => showPage('main'));
(document.getElementById('btn-back-gestures') as HTMLButtonElement).addEventListener('click', () => showPage('main'));

function setRoomId(id: string) {
  roomId = id;
  roomCodeEl.textContent = id;
  headerRoomCodeEl.textContent = id;
}
...
const ICON_PAUSE  = `<svg width="16" height="16" viewBox="0 0 256 256" fill="currentColor"><path d="M200,32H160a16,16,0,0,0-16,16V208a16,16,0,0,0,16,16h40a16,16,0,0,0,16-16V48A16,16,0,0,0,200,32Zm0,176H160V48h40ZM96,32H56A16,16,0,0,0,40,48V208a16,16,0,0,0,16,16H96a16,16,0,0,0,16-16V48A16,16,0,0,0,96,32Zm0,176H56V48H96Z"/></svg> Pause`;
const ICON_PLAY   = `<svg width="16" height="16" viewBox="0 0 256 256" fill="currentColor"><path d="M232.4,114.49,88.32,26.35a16,16,0,0,0-16.2-.3A15.86,15.86,0,0,0,64,39.87V216.13A15.86,15.86,0,0,0,72.12,230a16,16,0,0,0,16.2-.3L232.4,141.51a15.91,15.91,0,0,0,0-27ZM80,208.94V47.06L224,128Z"/></svg> Resume`;
