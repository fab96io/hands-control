...
(() => {
  ...
  async function startTracker(roomId) {
    ...
    const handLandmarker = await Pc.createFromOptions(vision, {
    ...
    function detect() {
      ...
      if (video.readyState >= 2 && video.currentTime !== lastVideoTime) {
        ...
        ctx.restore();
        const now = Date.now();
        if (now - lastSendTs >= THROTTLE_MS && ws2?.readyState === WebSocket.OPEN) {
          lastSendTs = now;
          const hands = results.landmarks.map((lm, i2) => ({
            handedness: results.handedness[i2]?.[0]?.categoryName ?? "Right",
            landmarks: lm.map((p2) => ({ x: p2.x, y: p2.y, z: p2.z }))
          }));
          ...
          if (primary) {
            ...
            const palmX = palmIdxs.reduce((s2, i2) => s2 + primary.landmarks[i2].x, 0) / palmIdxs.length;
            const palmY = palmIdxs.reduce((s2, i2) => s2 + primary.landmarks[i2].y, 0) / palmIdxs.length;
          ...
          updateGesture(
            hands.length === 0 ? "no hands" : pinch ? "pinch" : fist ? "fist" : peace ? "peace" : point ? "point \u2014 select" : openPalm ? "open palm" : zoom !== null ? `zoom (dist: ${zoom.toFixed(2)})` : isPanning ? "pan (3 fingers)" : "idle"
          );
        }
      }
